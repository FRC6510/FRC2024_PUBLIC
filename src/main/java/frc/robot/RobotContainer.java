...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    driverController.rightBumper().and(driverController.leftBumper()).onTrue(new InstantCommand(
    ...
    .onTrue(new InstantCommand(() -> shooterPivot.setIdle(NeutralModeValue.Coast), shooterPivot));
    // .whileFalse(stopFeedShooter);

    driverController.leftTrigger(0.1)
    .onTrue(new InstantCommand(() -> shooterPivot.setIdle(NeutralModeValue.Brake), shooterPivot));
    // .whileFalse(stopFeedShooter);

    
    //TODO operator controllers start here
    shooterPivot.setDefaultCommand(shooterPivot.shooterPivotAutomation(shooterPivot));
    
    operatorController.leftBumper().onTrue(
      new ParallelCommandGroup(
        feeder.spinFeeder(() -> 40),
        intake.spinIntake(() -> -40, lift)
      )
    ...
    //     new InstantCommand(() -> intake.stop(), intake),
    //     new InstantCommand(() -> feeder.stop(), feeder),
    //     shooterLimelight.ShooterLED(LEDMode.OFF)));


    operatorController.leftTrigger(0.1).onTrue(
      new ParallelCommandGroup(
        feeder.spinFeeder(() -> -40),
        shooter.shooterPID(() -> 1/3),
        shooterFeeder.spinShooterFeeder(() -> 0.175),
        new InstantCommand(() -> shooterPivot.setPivotPosition(20), shooterPivot)
      )).onFalse(shooterPivot.shooterPivotAutomation(shooterPivot));

    operatorController.rightTrigger(0.1)
      .onTrue(spinFeedShooter);
    ...
    operatorController.x().onTrue(
      shooterPivot.shooterPivotPID(shooterPivot, () -> 0)
    ).onFalse(shooterPivot.shooterPivotAutomation(shooterPivot));

      // operatorController.b().onTrue(feeder.spinFeeder(() -> -10)).onFalse(new InstantCommand(() -> feeder.stop(), feeder));
    ...
    operatorController.pov(180).onTrue(lift.liftPID(lift, () -> 19.5));
