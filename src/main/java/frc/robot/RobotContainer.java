...
import frc.robot.subsystems.lift.LiftPID;
...
public class RobotContainer {
  ...
  // Swerve Stuff
  public ShooterLimelight shooterLimelight = new ShooterLimelight();
  ...
  private void configureBindings() {
    ...
    operatorController.leftBumper().onTrue(lift.liftPID(lift, -90)); //position for amp
    ...
    //shooter and feeder spin together
    ...
    operatorController.leftTrigger().onTrue(new ShooterPID(shooter, () -> -0.95)).onFalse(new SpinShooter(shooter, 0));
    ...
    //shooter and feeder spin together, then move lift upwards
    ...
    operatorController.x().onTrue(shooterFeeder.spinShooterFeeder(0.5));

    //feeder, shooter feeder and shooter parallel
    //operatorController.x().onTrue(Commands.parallel(shooterFeeder.spinShooterFeeder(0.5), feeder.simpleSpinFeeder(-0.5), new ShooterPID(shooter, () -> 1))).onFalse(Commands.parallel(shooterFeeder.spinShooterFeeder(0), feeder.simpleSpinFeeder(0),new SpinShooter(shooter, 0)));
