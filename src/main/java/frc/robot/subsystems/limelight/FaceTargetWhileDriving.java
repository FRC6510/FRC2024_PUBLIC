...
public class FaceTargetWhileDriving extends Command{
  ...
  public FaceTargetWhileDriving(SwerveDrive swerveDrive, LimelightShooter limelightShooter, DoubleSupplier xSupplier, DoubleSupplier ySupplier, DoubleSupplier thetaSupplier){
    ...
    this.pidController= new PIDController(0.000120, 0, 0);//0.000118, 0, 0
  }
  ...
  public void initialize() {
    ...
    swerveDrive.fieldCentricDrive(
			deadzone(xSupplier.getAsDouble(), 0.1) * SwerveConstants.drivetrainMotorInfo.ROBOTMAXSPEED_DRIVE * SwerveDrive.speedLimit, //multiply each velocity by max velocity metres per second
			deadzone(ySupplier.getAsDouble(), 0.1) * SwerveConstants.drivetrainMotorInfo.ROBOTMAXSPEED_DRIVE * SwerveDrive.speedLimit,
			power * SwerveDrive.speedLimit
