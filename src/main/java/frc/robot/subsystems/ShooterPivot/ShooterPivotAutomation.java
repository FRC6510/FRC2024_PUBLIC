...
import frc.robot.commands.Autonomous.FieldCoordinates;
...
public class ShooterPivotAutomation extends Command {
  ...
  public ShooterPivotAutomation(SwerveDrive swerveDrive, ShooterPivot shooterPivot, double manualPos) {
  ...
  public void execute() {
    // NetworkTable table = NetworkTableInstance.getDefault().getTable("limelight-shooter");
      ...
      distanceFromSpeaker = Math.abs(-FieldCoordinates.DISTANCE_CENTRE_TO_ALLIANCE-swerveDrive.getPose2d().getX());//8

    if (operatorController.getBButton()){
      shooterPivot.setPivotPositionDegrees(0);
    } else {
      ...
      double pivotSetpoint = 19.326 * Math.log(distanceFromSpeaker) - 3.6895;
      // double pivotSetpoint = 064636*Math.pow(distanceFromSpeaker, 3) - 6.0271*Math.pow(distanceFromSpeaker, 2) + 27.305*distanceFromSpeaker - 22.36;

      shooterPivot.setPivotPositionDegrees(pivotSetpoint);
