...
public class MotionProfiling extends Command {
	...
	public void execute() {
		...
		atGoalpoint = 
		Math.abs(thetaSetpoint.getRadians() - swerveDrive.getPose2d().getRotation().getRadians()) <= (Math.PI / 180) * 1 &&
		Math.abs(xGoalpoint - swerveDrive.getPose2d().getX()) <= 0.03 &&
		Math.abs(yGoalpoint - swerveDrive.getPose2d().getY()) <= 0.03;
