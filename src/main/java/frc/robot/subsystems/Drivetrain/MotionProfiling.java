...
public class MotionProfiling extends Command {
	...
	private double calculateTrajectoryAngle(){
		...
		double yLength = yGoalpoint - swerveDrive.swerveDriveOdometry.getPoseMeters().getY();
		double xLength = xGoalpoint - swerveDrive.swerveDriveOdometry.getPoseMeters().getX();
	...
	private double calculateTotalDistance(){
		return Math.sqrt(
			Math.pow(xGoalpoint - swerveDrive.swerveDriveOdometry.getPoseMeters().getX(),2) +
			Math.pow(yGoalpoint - swerveDrive.swerveDriveOdometry.getPoseMeters().getY(),2));
	}

	private double calculateTotalHeading(){
		return swerveDrive.swerveDriveOdometry.getPoseMeters().getRotation().getDegrees();
	}
	...
	public void initialize() {
		...
		previousPosition = swerveDrive.swerveDriveOdometry.getPoseMeters();
	...
	public void execute() {
		...
		currentHeading = 0;
		currentPosition = swerveDrive.swerveDriveOdometry.getPoseMeters();
		...
		Pose2d setpointPose = new Pose2d(
			swerveDrive.swerveDriveOdometry.getPoseMeters().getX() + setpointState.position * Math.cos(calculateTrajectoryAngle()),
			swerveDrive.swerveDriveOdometry.getPoseMeters().getY() + setpointState.position * Math.sin(calculateTrajectoryAngle()),
		...
		atGoalpoint = 
		Math.abs(thetaSetpoint.getRadians() - swerveDrive.getPose2d().getRotation().getRadians()) <= (Math.PI / 180) * 1.5 &&
		Math.abs(xGoalpoint - swerveDrive.getPose2d().getX()) <= 0.05 &&
		Math.abs(yGoalpoint - swerveDrive.getPose2d().getY()) <= 0.05;
