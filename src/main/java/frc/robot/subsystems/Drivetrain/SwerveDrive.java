...
public class SwerveDrive extends SubsystemBase {
  ...
  public static double estimatedX;
  public static double estimatedY;
  public static double estimatedRot;
  ...
  public static double speedLimit = 1.0;
    ...
    public static SwerveDrivePoseEstimator poseEstimatorOdometry;
  ...
  public void setPower(double power){
    frontRight.setDrivePower(power);
  }
  ...
  public Command turnTo(Rotation2d heading) { 
    return new MotionProfiling(this, () -> poseEstimatorOdometry.getEstimatedPosition().getX(), () -> poseEstimatorOdometry.getEstimatedPosition().getY(), () -> heading);
  }
  ...
  public Command moveTo(double x, double y) { 
    return new MotionProfiling(this, () -> x, () -> y, () -> poseEstimatorOdometry.getEstimatedPosition().getRotation());
  }
  ...
  public Pose2d getPose2d(){
    return new Pose2d(
      poseEstimatorOdometry.getEstimatedPosition().getX(), 
      poseEstimatorOdometry.getEstimatedPosition().getY(), 
      poseEstimatorOdometry.getEstimatedPosition().getRotation());
  }
  ...
  public void periodic() {
    ...
    modulesPeriodic();
    Pose3d botPosition = limelightShooter.getBotPose(); 

    SmartDashboard.putString("Alliance", DriverStation.getAlliance().get().toString());

    SmartDashboard.putNumber("bot x", botPosition.getX());
    SmartDashboard.putNumber("bot y", botPosition.getY());
    SmartDashboard.putNumber("bot z", botPosition.getZ());
    SmartDashboard.putNumber("bot angle degree", Math.toDegrees(botPosition.getRotation().getAngle()));

    SmartDashboard.putNumber("estimated x",poseEstimatorOdometry.getEstimatedPosition().getX());
    SmartDashboard.putNumber("estimated y",poseEstimatorOdometry.getEstimatedPosition().getY());
    SmartDashboard.putNumber("estimated yaw (rotation)",poseEstimatorOdometry.getEstimatedPosition().getRotation().getDegrees());

    SmartDashboard.putNumber("imu angle", imu.getAngle());

    estimatedX = poseEstimatorOdometry.getEstimatedPosition().getX();
    estimatedY = poseEstimatorOdometry.getEstimatedPosition().getY();
    estimatedRot = Math.toDegrees(poseEstimatorOdometry.getEstimatedPosition().getRotation().getDegrees());
    ...
    if(limelightShooter.tagCount() >= 2 && DriverStation.isTeleopEnabled()){
      if (DriverStation.getAlliance().get() == Alliance.Red) {
        poseEstimatorOdometry.addVisionMeasurement(limelightShooter.getBotPose().toPose2d().rotateBy(new Rotation2d(180)), Timer.getFPGATimestamp() - difference);
      }
      else {
