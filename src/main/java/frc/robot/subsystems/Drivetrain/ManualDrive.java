...
public class ManualDrive extends Command {
	...
	public void execute() {
		swerveDrive.fieldCentricDrive(
			deadzone(xSupplier.getAsDouble(), 0.2) * SwerveConstants.drivetrainMotorInfo.ROBOTMAXSPEED, //multiply each velocity by max velocity metres per second
			deadzone(ySupplier.getAsDouble(), 0.2) * SwerveConstants.drivetrainMotorInfo.ROBOTMAXSPEED,
			deadzone(thetaSupplier.getAsDouble(), 0.2) * SwerveConstants.drivetrainMotorInfo.ROBOTMAXSPEED
		);
	}
