...
public class MotionCurving extends Command {
	 ...
	 */
	MotionCurving(SwerveDrive swerveDrive, DoubleSupplier xSupplier, DoubleSupplier ySupplier, double targetVelocity, Supplier<Rotation2d> headingSupplier) {
	...
	public MotionCurving(SwerveDrive swerveDrive, Pose2d target, double targetVelocity){
		this(swerveDrive, () -> target.getX(), () -> target.getY(), targetVelocity, () -> target.getRotation());
	}
	...
	private double calculateTrajectoryAngle(){
		...
		double yLength = yGoalpoint - swerveDrive.getPose2d().getY();
		double xLength = xGoalpoint - swerveDrive.getPose2d().getX();
	...
	private double calculateTotalDistance(){
		return Math.sqrt(
			Math.pow(xGoalpoint - swerveDrive.getPose2d().getX(),2) +
			Math.pow(yGoalpoint - swerveDrive.getPose2d().getY(),2));
	}

	private double calculateTotalHeading(){
		return swerveDrive.getPose2d().getRotation().getDegrees();
	}
	...
	public void initialize() {
		...
		previousPosition = swerveDrive.getPose2d();
	...
	public void execute() {
		...
		currentHeading = 0;
		currentPosition = swerveDrive.getPose2d();
		goalState = new TrapezoidProfile.State(totalDistance - currentPositionScalar,targetVelocity);// note that totalDistance changes with odometry. Trapezoidal profile takes in scalar quantities. 
		...
		Pose2d setpointPose = new Pose2d(
			swerveDrive.getPose2d().getX() + setpointState.position * Math.cos(calculateTrajectoryAngle()),
			swerveDrive.getPose2d().getY() + setpointState.position * Math.sin(calculateTrajectoryAngle()),
